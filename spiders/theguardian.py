...
class TheguardianSpider(scrapy.Spider):
    ...
    start_urls = ['http://theguardian.com/business/banking/',
                  'http://theguardian.com/technology/askjack/',
                  'http://theguardian.com/sport/formulaone/']

    def parse(self, response):
        
        #extract links from articles from each page
        for link in response.xpath('//div[@class="fc-item__content "]//a/@href').extract():
            yield scrapy.Request(link,callback=self.parse_guardian_contents)
    ...
    def parse_guardian_contents(self, response):
